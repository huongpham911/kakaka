...
import type { Project, VideoClip } from "../../shared/types";
import {
  RESOLUTIONS,
  AUTOSAVE_INTERVAL,
  STORAGE_KEYS,
...
const INTRO_PREVIEW_ID = "__intro__";
const OUTRO_PREVIEW_ID = "__outro__";
const TIMELINE_MIN_HEIGHT = 160;
const TIMELINE_MAX_HEIGHT = 420;
const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));

const getFilePath = (file: File): string => ((file as any).path ?? file.name ?? "").trim();
const toVideoSrc = (src: string): string => {
  if (!src) return src;
...
const measureVideoDuration = (opts: { file?: File; path?: string }): Promise<number | null> => {
  return new Promise((resolve) => {
    ...
    if (opts.file) {
      ...
      video.src = toVideoSrc(opts.path);
    } else {
      resolve(null);
      return;
    }
...
export default function App() {
  ...
  const timelineResizeMetaRef = React.useRef<{ startY: number; startHeight: number }>({ startY: 0, startHeight: 240 });
  const isTimelineResizingRef = React.useRef<boolean>(false);
  ...
  const previewUrlMap = React.useRef<Record<string, string>>({});
  const getPreviewSrc = useCallback((clip: Pick<VideoClip, 'id' | 'src'>) => {
    return previewUrlMap.current[clip.id] ?? toVideoSrc(clip.src);
  }, []);

  const revokePreviewUrl = useCallback((clipId: string) => {
    const url = previewUrlMap.current[clipId];
    if (url) {
      URL.revokeObjectURL(url);
      delete previewUrlMap.current[clipId];
    }
  }, []);

  useEffect(() => {
    return () => {
      Object.values(previewUrlMap.current).forEach((url) => URL.revokeObjectURL(url));
      previewUrlMap.current = {};
    };
  }, []);
  ...
  const resolveFilePath = useCallback((file: File, label: string): string | null => {
    const path = getFilePath(file);
    if (!path) {
      showToast('error', `Không thể đọc đường dẫn ${label}. Vui lòng kéo & thả file trực tiếp từ Explorer.`);
      return null;
    }
    return path;
  }, [showToast]);
  ...
  useEffect(() => {
    if (previewClipId && !t.video.find((clip) => clip.id === previewClipId)) {
      setPreviewClipId(null);
    }
  }, [previewClipId, t.video]);

  useEffect(() => {
    if (!previewClipId && t.video.length > 0) {
      setPreviewClipId(t.video[0].id);
    }
  }, [previewClipId, t.video]);
  ...
  useEffect(() => {
    ...
    t.video.forEach((clip) => {
      if (previewUrlMap.current[clip.id]) validIds.add(clip.id);
    });
    if (t.intro?.src && previewUrlMap.current[INTRO_PREVIEW_ID]) {
      validIds.add(INTRO_PREVIEW_ID);
    }
    if (t.outro?.src && previewUrlMap.current[OUTRO_PREVIEW_ID]) {
      validIds.add(OUTRO_PREVIEW_ID);
    }
    Object.keys(previewUrlMap.current).forEach((id) => {
      if (!validIds.has(id)) {
        URL.revokeObjectURL(previewUrlMap.current[id]);
        delete previewUrlMap.current[id];
      }
    });
  }, [t.video, t.intro?.src, t.outro?.src]);

  const currentClip = selectedClips.length > 0 ? t.video.find(c => c.id === selectedClips[0]) : null;
  const currentClipIndex = currentClip ? t.video.findIndex(c => c.id === currentClip.id) : -1;
  const currentClipLabel = currentClip
    ? (currentClip.src.split('/').pop() || currentClip.src.split('\\').pop() || currentClip.src)
    : '';
  const previewClipFromState = previewClipId ? t.video.find(c => c.id === previewClipId) : null;
  const activePreviewClip = previewClipFromState ?? t.video[0] ?? null;
  const activePreviewSrc = activePreviewClip ? getPreviewSrc(activePreviewClip) : "";


  const beginTimelineResize = useCallback((clientY: number) => {
    timelineResizeMetaRef.current = { startY: clientY, startHeight: timelineHeight };
    isTimelineResizingRef.current = true;
    setIsResizingTimeline(true);
  }, [timelineHeight]);

  const handleTimelineResizeMouseDown = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    event.preventDefault();
    beginTimelineResize(event.clientY);
  }, [beginTimelineResize]);

  const handleTimelineResizeTouchStart = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    if (!event.touches[0]) return;
    event.preventDefault();
    beginTimelineResize(event.touches[0].clientY);
  }, [beginTimelineResize]);
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: MouseEvent | TouchEvent) => {
      ...
      const delta = clientY - timelineResizeMetaRef.current.startY;
      const nextHeight = clamp(
        timelineResizeMetaRef.current.startHeight - delta,
        TIMELINE_MIN_HEIGHT,
        TIMELINE_MAX_HEIGHT
      );
    ...
    window.addEventListener("mouseup", stopTimelineResize);
    window.addEventListener("touchmove", handlePointerMove, touchOptions);
    window.addEventListener("touchend", stopTimelineResize);

    return () => {
      window.removeEventListener("mousemove", handlePointerMove);
      window.removeEventListener("mouseup", stopTimelineResize);
      window.removeEventListener("touchmove", handlePointerMove, touchOptions);
      window.removeEventListener("touchend", stopTimelineResize);
    };
  }, []);
  ...
  const addVideoClip = useCallback(async (filePath: string, file?: File) => {
    ...
    const measuredDuration = await measureVideoDuration({ file, path: filePath });
    const clipId = `clip-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    let added = false;
    let clipDuration = measuredDuration ?? 5;
    ...
    setP((current) => {
      const countValidation = validateClipCount(current.tracks.video.length + 1);
    ...
    if (added) {
      setSelectedClips([clipId]);
      setPreviewClipId((prev) => prev ?? clipId);
      if (previewUrl) {
        previewUrlMap.current[clipId] = previewUrl;
      }
      showToast('success', 'Video clip added successfully');
    } else if (previewUrl) {
      URL.revokeObjectURL(previewUrl);
    }
  }, [showToast, setP, setPreviewClipId]);
  ...
  }, [showToast, revokePreviewUrl]);

  const removeSelectedClips = useCallback(() => {
    if (selectedClips.length === 0) return;
    selectedClips.forEach(revokePreviewUrl);
    setP(s => ({
      ...s,
      tracks: {
        ...s.tracks,
    ...
    setSelectedClips([]);
  }, [selectedClips, showToast, revokePreviewUrl]);
  ...
  const handleDrop = async (e: React.DragEvent, type: 'video' | 'intro' | 'outro' | 'logo' | 'font' | 'bgm' | 'voice') => {
    e.preventDefault();
    e.stopPropagation();
    setDragOver(null);
    ...
    const labels: Record<'video' | 'intro' | 'outro' | 'logo' | 'font' | 'bgm' | 'voice', string> = {
      video: 'video',
      intro: 'intro video',
      outro: 'outro video',
      logo: 'logo image',
    ...
    const filePath = resolveFilePath(file, labels[type]);
    if (!filePath) return;
    ...
    switch (type) {
      case 'video':
        await addVideoClip(filePath, file);
        break;
      case 'intro': {
        const duration = await measureVideoDuration({ file, path: filePath });
        setP(old => ({
          ...old,
          tracks: { ...old.tracks, intro: { src: filePath, duration: duration ?? (old.tracks.intro?.duration || 3) } }
        }));
        revokePreviewUrl(INTRO_PREVIEW_ID);
        if (file) {
          previewUrlMap.current[INTRO_PREVIEW_ID] = URL.createObjectURL(file);
        }
      ...
      case 'outro': {
        ...
        setP(old => ({
          ...old,
          tracks: { ...old.tracks, outro: { src: filePath, duration: duration ?? (old.tracks.outro?.duration || 3) } }
        }));
        revokePreviewUrl(OUTRO_PREVIEW_ID);
        if (file) {
          previewUrlMap.current[OUTRO_PREVIEW_ID] = URL.createObjectURL(file);
        }
  ...
  const handlePreviewMetadata = useCallback(() => {
    if (!videoRef.current || !activePreviewClip) return;
    const realDuration = videoRef.current.duration;
    if (!isFinite(realDuration) || realDuration <= 0) return;
    const clipId = activePreviewClip.id;
    const clipSrc = activePreviewClip.src;
    setP((current) => {
      let updated = false;
      const videoClips = current.tracks.video.map((clip) => {
        if (clip.id === clipId && (!clip.duration || Math.abs(clip.duration - realDuration) > 0.05)) {
          updated = true;
          return { ...clip, duration: realDuration };
        }
        return clip;
      });
      let intro = current.tracks.intro;
      if (intro?.src === clipSrc && (!intro.duration || Math.abs(intro.duration - realDuration) > 0.05)) {
        intro = { ...intro, duration: realDuration };
        updated = true;
      }
      let outro = current.tracks.outro;
      if (outro?.src === clipSrc && (!outro.duration || Math.abs(outro.duration - realDuration) > 0.05)) {
        outro = { ...outro, duration: realDuration };
        updated = true;
      }
      return updated ? { ...current, tracks: { ...current.tracks, video: videoClips, intro, outro } } : current;
    });
  }, [activePreviewClip, setP]);
        ...
        {activeTab === 'media' && (
          <div className="tab-content">
            ...
            <div className="section">
              ...
              <label>Main timeline clips</label>
              <div className={`dropzone ${dragOver === "video" ? "drag-over" : ""}`}
                ...
                <input
                  ...
                  onChange={async e => {
                    ...
                    for (const f of files) {
                      const filePath = resolveFilePath(f, "video");
                      if (filePath) await addVideoClip(filePath, f);
                    }
                  }}
                />
                <div className="file-name">
                  {t.video.length > 0 ? `${t.video.length} clip${t.video.length > 1 ? "s" : ""} ready` : "No clips yet"}
                </div>
                <div className="drop-hint">Drop or browse video files</div>
              </div>

              {t.video.length > 0 && (
                <div className="video-clip-grid">
                  {t.video.map((clip, idx) => {
                    const fileName = clip.src.split("/").pop() || clip.src.split("\\").pop() || clip.src;
                    const clipSrc = getPreviewSrc(clip);
                    return (
                      <div key={clip.id} className="video-clip-card">
                        ...
                        >
                          <video
                            ...
                            preload="metadata"
                          />
                          <button className="thumb-play-btn">▶</button>
                        </div>
                        <div className="video-clip-card-meta">
                          <div className="video-clip-name">{fileName}</div>
                          <div className="video-clip-meta">#{idx + 1} • {(clip.duration || 5)}s • {clip.transition?.type || "fade"}</div>
                        </div>
                        <div className="video-clip-actions">
                          <button onClick={() => setSelectedClips([clip.id])}>Edit</button>
                          <button onClick={() => removeVideoClip(clip.id)}>Remove</button>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className={`dropzone ${dragOver === "intro" ? "drag-over" : ""}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, "intro"); }}>
                <input
                  type="file"
                  accept="video/*"
                  onChange={async e => {
                    ...
                    input.value = "";
                    if (!f) return;
                    const filePath = resolveFilePath(f, "intro video");
                    if (!filePath) return;
                    const duration = await measureVideoDuration({ file: f, path: filePath });
                    setP(old => ({
                      ...old,
                      tracks: {
                        ...old.tracks,
                        intro: { src: filePath, duration: duration ?? (old.tracks.intro?.duration || 3) }
                      }
                    }));
                    revokePreviewUrl(INTRO_PREVIEW_ID);
                    const previewUrl = URL.createObjectURL(f);
                    previewUrlMap.current[INTRO_PREVIEW_ID] = previewUrl;
                    showToast("success", "Intro video added");
                  }}
                />
                {t.intro?.src && <div className="file-name">🎬 {t.intro.src.split("/").pop()}</div>}
                <div className="drop-hint">Drop intro clip (plays before timeline)</div>
              </div>
              {t.intro?.src && (
                <div className="row">
                  <div>
                    <label>Intro Duration (s)</label>
                    <input
                      type="number"
                      min="1"
                      step="0.5"
                      value={t.intro?.duration ?? 3}
                      onChange={e => {
                        t.intro!.duration = Number(e.target.value) || 3;
                        setP({ ...p });
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <button className="btn-link" onClick={() => {
                      revokePreviewUrl(INTRO_PREVIEW_ID);
                      setP(old => ({ ...old, tracks: { ...old.tracks, intro: undefined } }));
                    }}>Remove intro</button>
                  </div>
                </div>
              )}
              ...
              <div className={`dropzone ${dragOver === "outro" ? "drag-over" : ""}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, "outro"); }}>
                <input
                  type="file"
                  accept="video/*"
                  onChange={async e => {
                    ...
                    input.value = "";
                    if (!f) return;
                    const filePath = resolveFilePath(f, "outro video");
                    if (!filePath) return;
                    const duration = await measureVideoDuration({ file: f, path: filePath });
                    setP(old => ({
                      ...old,
                      tracks: {
                        ...old.tracks,
                        outro: { src: filePath, duration: duration ?? (old.tracks.outro?.duration || 3) }
                      }
                    }));
                    revokePreviewUrl(OUTRO_PREVIEW_ID);
                    const previewUrl = URL.createObjectURL(f);
                    previewUrlMap.current[OUTRO_PREVIEW_ID] = previewUrl;
                    showToast("success", "Outro video added");
                  }}
                />
                {t.outro?.src && <div className="file-name">🎬 {t.outro.src.split("/").pop()}</div>}
                <div className="drop-hint">Drop outro clip (plays after timeline)</div>
              </div>
              {t.outro?.src && (
                <div className="row">
                  <div>
                    <label>Outro Duration (s)</label>
                    <input
                      type="number"
                      min="1"
                      step="0.5"
                      value={t.outro?.duration ?? 3}
                      onChange={e => {
                        t.outro!.duration = Number(e.target.value) || 3;
                        setP({ ...p });
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <button className="btn-link" onClick={() => {
                      revokePreviewUrl(OUTRO_PREVIEW_ID);
                      setP(old => ({ ...old, tracks: { ...old.tracks, outro: undefined } }));
                    }}>Remove outro</button>
                  </div>
                </div>
              )}
            ...
            <div className="section">
              ...
              <div className={`dropzone ${dragOver === 'logo' ? 'drag-over' : ''}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, 'logo'); }}>
                <input type="file" accept="image/*" onChange={e => {
                  const f = e.target.files?.[0]; if (!f) return;
                  const filePath = resolveFilePath(f, "logo image");
                  if (!filePath) return;
                  t.logo!.src = filePath;
                  setP({ ...p });
                }} />
            ...
            <div className="section">
              ...
              <div className={`dropzone ${dragOver === 'bgm' ? 'drag-over' : ''}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, 'bgm'); }}>
                <input type="file" accept="audio/*" onChange={e => {
                  const f=e.target.files?.[0]; if(!f) return;
                  const filePath = resolveFilePath(f, "background music");
                  if (!filePath) return;
                  t.audio!.bgm!.src=filePath; setP({ ...p });
                }} />
              ...
              <div className={`dropzone ${dragOver === 'voice' ? 'drag-over' : ''}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, 'voice'); }}>
                <input type="file" accept="audio/*" onChange={e => {
                  const f=e.target.files?.[0]; if(!f) return;
                  const filePath = resolveFilePath(f, "voice-over");
                  if (!filePath) return;
                  t.audio!.voice!.src=filePath; setP({ ...p });
                }} />
            ...
            <div className="section">
              ...
              <div className={`dropzone ${dragOver === 'font' ? 'drag-over' : ''}`}
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => { void handleDrop(e, 'font'); }}>
                <input type="file" accept=".ttf,.otf" onChange={e => {
                  const f = e.target.files?.[0]; if (!f) return;
                  const filePath = resolveFilePath(f, "font");
                  if (!filePath) return;
                  t.ticker!.font = filePath; setP({ ...p });
                }} />
        ...
        <div className="timeline-header">
          <div className="timeline-info">
            ...
            {selectedClips.length > 0 && (
              <div className="timeline-selected-chip">
                <span>{selectedClips.length} selected</span>
                <button onClick={() => setSelectedClips([])}>Clear</button>
              </div>
            )}
          </div>
          ...
          <div
            ...
            onTouchStart={handleTimelineResizeTouchStart}
            role="separator"
            aria-orientation="horizontal"
            aria-label="Kéo để điều chỉnh chiều cao timeline"
            title="Kéo để thay đổi chiều cao timeline"
          >
            <div className="timeline-resize-grip" />
          </div>

          <div className="timeline-controls">
            <span className="timeline-height-label">
            ...
            <button
              className="btn-add-track compact"
              onClick={() => {
        ...
        <div className="timeline-tracks">
          ...
          <div className="track video-track">
            ...
            >
              ...
              {/* Intro */}
              {t.intro?.src && (
                <div className="track-item video-track-item" onClick={() => setSelectedClips([])}>
                  <div className="clip-thumb">
                    <video src={getPreviewSrc({ id: INTRO_PREVIEW_ID, src: t.intro.src })} muted playsInline preload="metadata" />
                  </div>
              ...
              {t.video.map((clip, idx) => {
                ...
                return (
                  <div key={clip.id}
                    className="track-item video-track-item"
                    draggable
                    ...
                    <div className="clip-thumb">
                      <video src={getPreviewSrc(clip)} muted playsInline preload="metadata" />
                    </div>
                    <div className="clip-info">
              ...
              <div className={`track-item ${dragOver === 'video' ? 'drag-over' : ''}`}
                ...
                onClick={() => {
                  ...
                  input.onchange = async (e: any) => {
                    ...
                    const files = target ? Array.from(target.files ?? []) : [];
              ...
              {t.outro?.src && (
                <div className="track-item video-track-item" onClick={() => setSelectedClips([])}>
                  <div className="clip-thumb">
                    <video src={getPreviewSrc({ id: OUTRO_PREVIEW_ID, src: t.outro.src })} muted playsInline preload="metadata" />
                  </div>
                  <div className="clip-info">

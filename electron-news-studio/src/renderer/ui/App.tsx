          ...
          <div className="preview-video">
            <div style={{textAlign: 'center', padding: '40px'}}>
              ...
              <p style={{fontSize: '14px', opacity: 0.6}}>
                {t.intro ? '• Intro ' : ''}
                {t.video.length > 0 ? `• ${t.video.length} clip${t.video.length > 1 ? 's' : ''} ` : ''}
                {t.outro ? '• Outro ' : ''}
              </p>
        ...
        <div className="timeline-clips">
          ...
          {!t.intro?.src && (
            <div className={`add-clip-btn ${dragOver === 'intro' ? 'drag-over' : ''}`}
              ...
              <span style={{fontSize: '12px'}}>Add Intro</span>
            </div>
          )}
          ...
          {t.video.map((clip, idx) => (
            <div key={clip.id}
              ...
              <div className="clip-controls" style={{marginTop: '8px'}}>
                <label>Duration: {clip.duration || 5}s</label>
                <label style={{fontSize: '11px', opacity: 0.7}}>
                  Transition: {clip.transition?.type || 'fade'} ({clip.transition?.duration || 1}s)
                </label>
              </div>
            </div>
          ))}
          ...
          {!t.outro?.src && (
            <div className={`add-clip-btn ${dragOver === 'outro' ? 'drag-over' : ''}`}
              ...
              <span style={{fontSize: '24px'}}>+</span>
              <span style={{fontSize: '12px'}}>Add Outro</span>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="controls-grid">
              ...
              {currentClip && (
                <div className="section">
                  <h3 className="section-title">✂️ Clip Editor: {t.video.findIndex(c => c.id === selectedClip) + 1}</h3>
                  <div className="row">
                    <div>
                      <label>Duration (s)</label>
              ...
              {/* Logo Section */}
              <div className="section">
                <h3 className="section-title">🖼️ Logo Overlay</h3>
                <div className={`dropzone ${dragOver === 'logo' ? 'drag-over' : ''}`}
                  ...
                  <div className="drop-hint">Kéo thả logo</div>
                </div>
                {t.logo?.src && (
                  <div className="row">
                    <div>
              ...
              <div className="section">
                ...
                {(t.audio?.bgm?.src || t.audio?.voice?.src) && (
                  <div className="row">
                    {t.audio?.bgm?.src && <div><label>BGM Gain (dB)</label><input type="number" value={t.audio?.bgm?.gain ?? -6} onChange={e => { t.audio!.bgm!.gain = Number(e.target.value||-6); setP({ ...p }); }} /></div>}
                    {t.audio?.voice?.src && <div><label>Voice Gain (dB)</label><input type="number" value={t.audio?.voice?.gain ?? 0} onChange={e => { t.audio!.voice!.gain = Number(e.target.value||0); setP({ ...p }); }} /></div>}
                  </div>
                )}
                {t.audio?.voice?.src && t.audio?.bgm?.src && (
                  <div>
                    <label>Duck BGM when voice plays</label>
              ...
              {/* Ticker Section */}
              <div className="section">
                <h3 className="section-title">📝 Ticker (Chữ Chạy)</h3>
                <textarea rows={2} value={t.ticker?.text ?? ""} onChange={e => { t.ticker!.text = e.target.value; setP({ ...p }); }}
                  placeholder="Nhập nội dung chữ chạy..." style={{fontSize: '13px'}} />
                ...
                <details>
                  <summary style={{cursor: 'pointer', fontSize: '12px', opacity: 0.85, marginBottom: '8px'}}>Advanced Ticker Settings</summary>
                  <div className="row">
                  ...
                  <div className="row">
                    <div><label>Text Color</label><input type="text" value={t.ticker?.color ?? "white"} onChange={e => { t.ticker!.color = e.target.value; setP({ ...p }); }} /></div>
                    <div><label>Text Opacity</label><input type="number" min="0" max="1" step="0.05" value={t.ticker?.textOpacity ?? 1.0} onChange={e => { t.ticker!.textOpacity = Number(e.target.value); setP({ ...p }); }} /></div>
                  </div>
                  <div className="row">
            ...
            <div className="controls-grid">
              ...
              <div className="section">
                ...
                <div className="row">
                  ...
                  <div><label>Duration (s)</label><input type="number" value={p.duration} onChange={e => set("duration", Number(e.target.value||15))} /></div>
                </div>
                <div className="row">
              ...
              <div className="section">
                ...
                <button onClick={onExport} disabled={disabled} style={{width: '100%', padding: '12px', fontSize: '16px'}}>
                  {disabled ? '⚠️ Add at least 1 video' : '🎬 Export MP4'}
                </button>
                <div className="hint">Output: output_news.mp4</div>
              </div>
            </div>
          </div>
        )}
